...
# give frontend backend access to storage account so it can read templates and configuration items
...
az role assignment create --assignee-object-id ${BACKEND_VMSS_PRINCIPAL_ID} --scope ${PRUNELLA_STORAGE_ACCOUNT_ID} --role Contributor
...
# scaling hosts
display_progress "Scaling backend"
...
# lets sleep 60 allowing backend to settle
sleep 60

# scaling hosts
display_progress "Scaling frontend"
